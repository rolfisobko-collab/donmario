...
export function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (isHomePage) {
        const sections = ["about", "rooms", "parrilla", "traslados", "contact"]
  ...
  const getMobileMenuStyle = () => {
    if (!isHomePage) {
      if (pathname.includes("restaurante")) {
        return "bg-gradient-to-br from-slate-50 to-red-50 border-slate-200"
      }
      if (pathname.includes("traslados")) {
        return "bg-gradient-to-br from-slate-50 to-amber-50 border-slate-200"
      }
      if (pathname.includes("habitaciones")) {
        return "bg-gradient-to-br from-slate-50 to-amber-50 border-slate-200"
      }
      return "bg-gradient-to-br from-slate-50 to-zinc-50 border-slate-200"
    }
  ...
  const getMobileLinkStyle = (link: { href: string; label: string; isPage?: boolean }) => {
    return link.isPage 
      ? "text-slate-700 font-medium bg-white border border-slate-200 hover:bg-slate-50" 
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Button variant="outline" size="sm" className={getButtonStyle()} asChild>
              <Link href="https://wa.me/5493757671408?text=Hola!%20Quiero%20hacer%20una%20reserva%20en%20Don%20Mario%20Turismo" target="_blank" rel="noopener noreferrer">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent className={`${getMobileMenuStyle()} border-0 shadow-2xl`}>
              ...
              <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-200">
                <Link 
                  href="/" 
                  className={`font-serif text-xl font-bold text-slate-700`}
